...
{
    size_t nameSize;
    clGetPlatformInfo(m_platformID, CL_PLATFORM_NAME, 0, NULL, &nameSize);
    char* name = new char[nameSize];
    clGetPlatformInfo(m_platformID, CL_PLATFORM_NAME, nameSize, name, 0);
    m_name = QString(name);
    delete[] name;
    clGetPlatformInfo(m_platformID, CL_PLATFORM_VERSION, 0, NULL, &nameSize);
    name = new char[nameSize];
    clGetPlatformInfo(m_platformID, CL_PLATFORM_VERSION, nameSize, name, 0);
    m_version = QString(name);
    delete name;
    clGetPlatformInfo(m_platformID, CL_PLATFORM_VENDOR, 0, NULL, &nameSize);
    name = new char[nameSize];
    clGetPlatformInfo(m_platformID, CL_PLATFORM_VENDOR, nameSize, name, 0);
    m_vendor = QString(name);
    delete name;
    clGetPlatformInfo(m_platformID, CL_PLATFORM_EXTENSIONS, 0, NULL, &nameSize);
    name = new char[nameSize];
    clGetPlatformInfo(m_platformID, CL_PLATFORM_EXTENSIONS, nameSize, name, 0);
    m_extension = QString(name);
    delete name;

    uint totalDevice;
    clGetDeviceIDs(m_platformID, deviceType, 0, NULL, &totalDevice);
    cl_device_id* devicesID =  new cl_device_id[totalDevice];
    clGetDeviceIDs(m_platformID, deviceType, totalDevice, devicesID, NULL);
    for(unsigned int i = 0; i < totalDevice; ++i)
    {
...
, m_numberDevices(0)
{
    ...
    for(unsigned int i = 0; i < m_numberDevices; ++i)
    {
